...
type Writer interface {
	Print(ctx context.Context, message string, value ...any)
}
...
func (w *DefaultWriter) Print(ctx context.Context, message string, value ...any) {
	l := GetLogger()
...
func safeHeaders(v any) http.Header {
...
func (l *Logger) Flush() {
	for range l.options.WorkerPoolSize {
		select {
...
func (l *Logger) maskComplexValue(val any, path string) any {
	if val == nil {
		return nil
	}
	if l.options.DisableMasking {
...
func (l *Logger) maskStruct(value reflect.Value) any {
...
func (l *Logger) maskMap(value reflect.Value) any {
	...
	for _, key := range value.MapKeys() {
		...
		// Unwrap any — penting untuk map[string]any yang merupakan
...
func (l *Logger) maskSlice(value reflect.Value) any {
...
func (l *Logger) Print(ctx context.Context, s string, v ...any) {
	if len(v) < 2 {
		return
	}
